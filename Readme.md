...
3) Prepare your working directory by either copying a docx file into the folder `work` or, alternatively, run `podman exec <your-container-name> /bin/bash -c "processDocx reset-example"` to use the demo docx file
...
Start the docx conversion with the following command:
    - `podman exec <your-container-name> /bin/bash -c "processDocx"`
...
`podman exec <your-container-name> /bin/bash -c "processDocx help`
...
`docker exec <your-container-name> /bin/bash -c "processDocx help <recipe-name>`
...
`podman exec <your-container-name> /bin/bash -c "processDocx html"`
...
`podman exec <your-container-name> /bin/bash -c "processDocx pagedjs weasyprint"`
...
`podman exec <your-container-name> /bin/bash -c "processDocx -f /root/xmlworkflow/themes/default/justfile help"`
...
`podman exec <your-container-name> /bin/bash -c "processDocx docxtojats html- weasyprint-"`
...
`podman exec <your-container-name> /bin/bash -c "processDocx xml-file='Dummy_Article_Template.docx_SaxonHE.xml' html- weasyprint-"`
...
`podman exec \<your-container-name> /bin/bash -c "processDocx runtests"
