<?xml version="1.0"?>
<!-- ============================================================= -->
<!-- XSLT for BerlinUP Jats XML to HTML conversion                 -->
<!-- based on NISO JatsPreviewStylesheets                          -->
<!-- modified by Ronald Steffen                                    -->
<!-- ============================================================= -->
...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="css" select="'media/jats_html.css'" />

  <xsl:param name="report-warnings" select="'yes'" />
          ...
          <div style="display: inline-block; width: 50%">
            ...
            <div>
              <xsl:variable name="doi">
                <xsl:apply-templates select="article-id[@pub-id-type='doi']"></xsl:apply-templates>
              </xsl:variable>
              <p>
                <a href="https://doi.org/{$doi}">https://doi.org/<xsl:apply-templates
                    select="article-id[@pub-id-type='doi']"></xsl:apply-templates>
                </a>
              </p>
            </div>
          </div>
  ...
  <xsl:template name="footer-metadata">
    ...
    <xsl:for-each select="front/article-meta | front-stub">
      <xsl:if
        test="article-categories | kwd-group | counts |
                    supplementary-material | custom-meta-group |
                    custom-meta-wrap">
    ...
    <xsl:call-template name="metadata-labeled-entry">
      <xsl:with-param name="label">
        ...
        <xsl:for-each select="@date-type">
          <xsl:choose>
            <xsl:when test=".='accepted'">Angenommen</xsl:when>
            <xsl:when test=".='received'">Eingereicht</xsl:when>
  ...
  <xsl:template name="issue-info">
    ...
    <xsl:choose>
      <xsl:when
        ...
        <!-- if there are only one issue, issue-id and issue-title and nothing else, we make one
        line only -->
        <xsl:call-template name="metadata-labeled-entry">
  ...
  <xsl:template match="title-group" mode="metadata">
    ...
    <xsl:apply-templates
      select="article-title | subtitle | trans-title-group |
  ...
  <xsl:template mode="metadata" match="article-meta/contrib-group">
    ...
    <xsl:if test="$misc-contrib-data">
      <div class="row">
        <div class="cell">
          <div class="metadata-group misc-contrib-data">
            <xsl:apply-templates mode="metadata"
  ...
  <xsl:template name="contrib-identify">
    ...
    name | name-alternatives | degrees | xref)
    and @equal-contrib -->
    <div class="metadata-group contrib-identify">
      <xsl:for-each
        select="anonymous |
  ...
  <xsl:template match="contrib-id[@contrib-id-type='orcid']" mode="metadata-inline">
  ...
  <xsl:template name="contrib-info">
    ...
    <div class="metadata-group contrib-info">
      <xsl:apply-templates mode="metadata"
        select="address | aff | author-comment | bio |
                ext-link | on-behalf-of | role | uri" />
    </div>
  </xsl:template>
  ...
  <xsl:template
    match="array | disp-formula-group | fig-group |
  ...
  <xsl:template
    match="boxed-text | chem-struct-wrap |
  ...
  <xsl:template match="caption">
    <div class="caption">
      <xsl:apply-templates select="./parent::node()/label" mode="label" />
      <xsl:apply-templates />
    </div>
  </xsl:template>
  ...
  <xsl:template match="permissions">
    <div class="permissions">
      ...
      <xsl:if test="copyright-year | copyright-holder">
        <p class="copyright">
          <span class="generated">
            <p>Lizenzinformation:</p>
          </span>
          <xsl:for-each select="copyright-year | copyright-holder">
            <xsl:if test="local-name() = 'copyright-year'"> &#xA9; </xsl:if>
            <xsl:apply-templates />
  ...
  <xsl:template match="license-p/graphic">
    <div>
      <xsl:variable name="url">
        <xsl:value-of select="//ali:license_ref" />
      </xsl:variable>
      <a href="{$url}" target="_blank">
        <img alt="" src="./media/ccby.png"></img>
      </a>
    </div>
  </xsl:template>

  <xsl:template match="ali:license_ref"></xsl:template>
  ...
  <xsl:template match="ref/note" priority="2">
    ...
    <xsl:if
      test="normalize-space(string($label))
  ...
  <xsl:template
    match="aff/label | corresp/label | chem-struct/label |
  ...
  <xsl:template
    match="app/label | boxed-text/label |
  ...
  <xsl:template
    match="fn-group/fn | table-wrap-foot/fn |
  ...
  <xsl:template name="named-anchor">
    ...
    <xsl:variable name="id">
      <xsl:choose>
        ...
        <xsl:when
          test="not(preceding-sibling::*) and
          ...
          <xsl:value-of
            select="(parent::alternatives | parent::name-alternatives |
            parent::citation-alternatives | parent::collab-alternatives |
            parent::aff-alternatives)/@id" />
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:if test="$verbose">
      ...
      <xsl:variable name="xref-warnings">
        <xsl:for-each select="//xref[not(normalize-space(string(.)))]">
          ...
          <xsl:if
            test="not(normalize-space(string($target-label))) and
